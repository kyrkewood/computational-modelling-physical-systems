...
int main()
{
	ofstream fout ("Project4d_output.dat");
	...
	for (t = h; t < 0.9999999999; t = t + h)
	{
		...
		ru2error = pow(((calculateds(y1, t) - run) * (calculateds(y1, t) - run)), 0.5)/calculateds(y1, t);
		ru4error = pow(((calculateds(y1, t) - ru4) * (calculateds(y1, t) - ru4)), 0.5)/calculateds(y1, t);
