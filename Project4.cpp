...
// Payloads project
// Program to solve the motion of a rocket under a uniform gravitational field with varying step sizes for RK algorithms
// Nicholas Kyrke-Smith, 21 February 2008
...
int main()
{
	ofstream fout ("Project4_output.dat");
	...
	cout << "log(h)\tAverage error using RU2\tAverage error using RU4\n";

	for (h = 0.1; h > 0.0000001; h = (h / 2))
	{
		double ru2error = 0, ru4error = 0;
		yr1 = y1, yr2 = y2, y41 = y1, y42 = y2;

		double a, t, counter = 0;
		for (t = h; t < 0.9999999999; t = t + h)
		{
			...
			ru2error = ru2error + pow(((calculateds(y1, t) - run) * (calculateds(y1, t) - run)), 0.5);
			ru4error = ru4error + pow(((calculateds(y1, t) - ru4) * (calculateds(y1, t) - ru4)), 0.5);
		}

		cout << (log(h) / log(10)) << "\t" << (ru2error / counter) << "\t" << (ru4error / counter) << "\n";
		fout << (log(h) / log(10)) << "\t" << (ru2error / counter) << "\t" << (ru4error / counter) << "\n";
	}
