...
int main()
{
	ofstream fout ("Project1b_output3.dat");
	...
	cout << "t\tCalculated\t2nd order Runge-Kutta\t4th order Runge-Kutta\n";

	double a, t;
	for (t = h; t < 0.9999999999; t = t + h)
	{
		//Obtain the values from the functions at this point in the loop
		rungekutta(h, yr1, yr2, t);
		rungekutta4(h, y41, y42, t);
		
		run = yr2;
		ru4 = y42;
		
		cout << t << "\t" << calculatedv(t) << "\t" << run << "\t" << ru4 << "\n";
		fout << t << "\t" << calculatedv(t) << "\t" << run << "\t" << ru4 << "\n";
	}
